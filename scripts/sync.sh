#!/bin/bash
# sync.sh — Reset current branch to match the default branch exactly.
# The branch becomes a clean copy of the default branch under the same name.
# All local commits on the branch are discarded.

set -e

branch=$(git symbolic-ref --short HEAD 2>/dev/null)
# Per-repo override: git config rally.syncBranch staging
# Otherwise auto-detect via origin/HEAD
